...
num_epochs = 35           # Number of full passes through the dataset
...
model = LSTM(in_dim=94, out_dim=94,hid_dim=50,batch_size=16,no_layers=1)
...
pickle.dump( results, open( "own_results_50.p", "wb" ) )
...
X=np.linspace(0,35,35).reshape((35,1))
total_loss = np.asarray(avg_minibatch_loss)
loss_val_list = np.asarray(loss_val_list)
Y_tr = total_loss.reshape((35,1))
Y_v = loss_val_list.reshape((35,1))
...
plt.ylabel('Loss')
plt.xlabel('Epochs')
plt.title('Validation and Training Loss with 50 Hidden Units')
ax_.legend()
plt.show()
...
# Instantiate a BasicCNN to run on the GPU or CPU based on CUDA support
model = LSTM(in_dim=94, out_dim=94,hid_dim=75,batch_size=16,no_layers=1)
model = model.to(computing_device)
print("Model on CUDA?", next(model.parameters()).is_cuda)
...
results = { "acc_train_list": acc_train_list, "acc_val_list": acc_val_list, "loss_train_list":total_loss,"loss_val_list":loss_val_list}
pickle.dump( results, open( "own_results_75.p", "wb" ) )
...
plt.ylabel('Loss')
plt.xlabel('Epochs')
plt.title('Validation and Training Loss with 75 Hidden Units')
ax_.legend()
plt.show()
...
# Instantiate a BasicCNN to run on the GPU or CPU based on CUDA support
model = LSTM(in_dim=94, out_dim=94,hid_dim=150,batch_size=16,no_layers=1)
model = model.to(computing_device)
print("Model on CUDA?", next(model.parameters()).is_cuda)
...
results = { "acc_train_list": acc_train_list, "acc_val_list": acc_val_list, "loss_train_list":total_loss,"loss_val_list":loss_val_list}
pickle.dump( results, open( "own_results_150.p", "wb" ) )
...
plt.ylabel('Loss')
plt.xlabel('Epochs')
plt.title('Validation and Training Loss with 150 Hidden Units')
ax_.legend()
plt.show()
...
import pickle
pickle_in = open("own_results_150.p","rb")
train_150 = pickle.load(pickle_in)
print(train_150)
